...
def train_lstm_embedding(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    for i, station in enumerate(stations):
        ...
        dataset_train, dataset_valid, normalizer_at, normalizer_wt = create_dataset_embedding(
            ...
            df_station,
            i,
            valid_use_window=valid_use_window,
            name=station,
    ...
    training_loop(
        ...
        normalizer_at=normalizers_at,
        normalizer_wt=normalizers_wt,
        settings=settings,
        verbose=verbose,
    )
...
def create_dataset_embedding(
    ...
    # Create datasets
    dataset_train = SequenceWindowedDataset(config["window_len"], df_train, embedding_idx=i, name=name, dev_run=dev_run)
    if valid_use_window:
...
def train_stgnn(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    dataloader_train = torch.utils.data.DataLoader(
        ...
        batch_size=config["batch_size"],
        shuffle=True,
        drop_last=False,  # todo: use True?
    )
    batch_size = 1 if isinstance(dataset_valid, STGNNSequenceFullDataset) else config["batch_size"]
    dataloader_valid = torch.utils.data.DataLoader(dataset_valid, batch_size=batch_size, shuffle=False, drop_last=False)
    ...
    model = SpatioTemporalEmbeddingModel(
        ...
        config["num_convs"],
        config["num_heads"],
        edge_hidden_size=config.get("edge_hidden_size"),
...
def train_transformer_embedding(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    model = TransformerEmbeddingModel(
        1,
        num_embeddings=num_embeddings if config.get("use_station_embedding", True) else 0,
        ...
        if config.get("d_model", None)
        else int(config["ratio_heads_to_d_model"] * config["num_t_heads"]),
...
def train_transformer_stgnn(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    model = SpatioTemporalEmbeddingModel(
        ...
        config["embedding_size"],
        config["hidden_size"]
        if config.get("hidden_size", None)
        else int(config["ratio_heads_to_d_model"] * config["num_t_heads"]),
        config["num_layers"],
        # Graph NN params:
        config["num_convs"],
...
if __name__ == "__main__":
    ...
    method = "lstm_embedding"  # 'lstm_embedding', 'stgnn', 'transformer_embedding', or 'transformer_stgnn'
