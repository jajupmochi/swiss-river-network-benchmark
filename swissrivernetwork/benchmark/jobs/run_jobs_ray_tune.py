...
def get_job_script(job_name: str, py_file: str, params: dict = {}, device: str = "gpu") -> str:
    if device == "gpu":
        ...
        fn_op = os.path.join(cur_path, "outputs/" + prefix_kw + "." + job_name + ".o" + now.strftime("%Y%m%d%H%M%S"))
...
python3 """
        + py_file
        + " "
        + " ".join([r"""--""" + k + r""" """ + str(v) for k, v in params.items()])
    )
    script = script.strip()
    script = re.sub("\n\t+", "\n", script)
    script = re.sub("\n +", "\n", script)
...
source """
        + cur_path
        + r"""/../../../.venv/bin/activate  # change as needed
python3 --version
module list
