...
from swissrivernetwork.benchmark.dataset import read_graph, read_stations
...
def plot_diff(
    ...
    plt.plot(epoch_days, mean_diff, color="tab:blue", linestyle="-", linewidth=1, label="Difference")
    plt.fill_between(
...
def evaluate_best_trial_single_model(graph_name, method, output_dir: Path | None = None, settings: dict = {}):
    ...
    if "stgnn" == method:
        ...
        model = SpatioTemporalEmbeddingModel(
            ...
            best_config["embedding_size"],
            best_config["hidden_size"]
            if best_config.get("hidden_size", None)
            else int(best_config["ratio_heads_to_d_model"] * best_config["num_t_heads"]),
            best_config["num_layers"],
            # Graph NN params:
            best_config["num_convs"],
            best_config["num_heads"],
    ...
    # Update extra_resu:
    test_resu[5]["train_time_total_best"] = [
        best_trial_all.last_result["time_total_s"] / num_embeddings
    ] * num_embeddings
...
def evaluate_best_trial_isolated_station(
    ...
    if method in ["lstm", "graphlet"]:
        ...
        model = TransformerModel(
            ...
            if best_config.get("d_model", None)
            else int(best_config["ratio_heads_to_d_model"] * best_config["num_t_heads"]),
            max_len=best_config["max_len"],
            missing_value_method=best_config["missing_value_method"],
            use_current_x=best_config["use_current_x"],
        ...
        model = TransformerEmbeddingModel(
            input_size,
            num_embeddings=num_embeddings if best_config["use_station_embedding"] else 0,
            embedding_size=best_config["embedding_size"],
            ...
            d_model=best_config["d_model"]
            if best_config.get("d_model", None)
            else int(best_config["ratio_heads_to_d_model"] * num_t_heads),
    ...
    if "lstm" == method:
        test_resu = test_lstm(
            ...
            window_len=window_len,
            dump_dir=DUMP_DIR,
            predict_dump_dir=predict_dump_dir,
            config=best_config,
            verbose=settings.get("verbose", 2),
        )
...
def process_method(graph_name, method, output_dir: Path | None = None, settings: dict = {}, return_extra: bool = False):
    ...
    if method in ["lstm", "graphlet", "lstm_embedding", "transformer", "transformer_graphlet", "transformer_embedding"]:
        if verbose > 1:
            spinner = Halo(
                text="Processing Stations ->", spinner="pong", color="cyan", text_color="grey", animation="bounce"
            )
            spinner.start()
    ...
    df = pd.DataFrame(
        data={"Station": col_station, "RMSE": col_rmse, "MAE": col_mae, "NSE": col_nse, "N": col_n, **col_extra_resu}
    )
...
if __name__ == "__main__":
    ...
    settings = {
        ...
        "extrapo_mode": "future_embedding",  # None, 'future_embedding'
        "noise_type": None,  # 'gaussian_a' or 'impulse_a' or None. Default None
    ...
    if GRAPH_RUN:
        ...
        for m in METHODS[4:5]:  # fixme: test only [2:3}
            process_method(
                graph_name, m, output_dir=OUTPUT_DIR, settings={**settings, **{"window_len": window_len_map[m]}}
            )
