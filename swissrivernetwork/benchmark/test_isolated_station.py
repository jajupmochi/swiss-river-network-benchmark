...
from swissrivernetwork.benchmark.model import LstmEmbeddingModel, LstmModel
...
def dump_predictions(
    ...
    if prediction.ndim == 1:
        ...
        if noise_type is None:
            df = pd.DataFrame(data={"epoch_day": epoch_days, f"{station}_wt_hat": prediction})
        else:
            ...
            df = pd.DataFrame(data={"epoch_day": epoch_days, f"{station}_wt_hat{col_key}": prediction})
...
def plot(
    ...
    if plot_diff:
        plt.plot(
            epoch_days,
            np.abs(np.array(actual) - np.array(prediction)),
            color="tab:red",
            linestyle="-",
            linewidth=0.5,
            label="Difference",
...
def compute_metrics(
    actual: np.ndarray, prediction: np.ndarray, extra_resu: dict, config: dict, station: str, verbose: int
...
def test_graphlet(
    ...
    df_neighs = [
        read_csv_prediction_test(graph_name, "lstm", neigh, predict_dump_dir=predict_dump_dir) for neigh in neighs
    ]
    if config.get("noise_type") is not None:
        df_neighs = [
            select_conditioned_columns(
...
def test_transformer_graphlet(
    ...
    predict_dump_dir = dump_dir if predict_dump_dir is None else predict_dump_dir
    df_neighs = [
        read_csv_prediction_test(graph_name, "transformer", neigh, predict_dump_dir=predict_dump_dir)
        for neigh in neighs
    ]
    if config.get("noise_type") is not None:
        df_neighs = [
