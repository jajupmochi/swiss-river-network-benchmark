...
from ray.tune import Callback, choice, randint, run, sample_from, uniform
...
from swissrivernetwork.benchmark.train_single_model import (
    train_lstm_embedding,
    train_masked_transformer_embedding,
    train_stgnn,
    train_transformer_embedding,
    train_transformer_stgnn,
)
from swissrivernetwork.benchmark.util import get_run_extra_key, get_run_name, is_transformer_model, str2bool
...
def run_experiment(
    ...
    num_cpus = (
        None
        if ("num_cpus" not in config or config.num_cpus is None)
        else (
            config.num_cpus
            if config.num_cpus > 0
            else os.cpu_count() + config.num_cpus  # todo: check this for clusters
        )
    )
    ray.init(
        num_cpus=num_cpus,
    ...
    if "lstm" == method or "graphlet" == method:
        ...
        for station in read_stations(graph_name):
            ...
            if "transformer" == method:
                ...
                trainer = partial(
                    train_transformer_graphlet,
                    settings={**config, "path_extra_keys": get_run_extra_key(config)},
                    verbose=verbose,
                )
            else:
                raise ValueError(f"Unknown method: {method}.")
    ...
    if if_trim_checkpoints:
        ...
        trim_checkpoints(
            ...
            keep_best_n=10,
            anchor_metric="validation_mse",
            mode="min",
            if_trim_best_n=True,
            keep_best_for_trimmed_trials=True,
            keep_last_for_trimmed_trials=False,
            verbose=False,
        )
...
def parse_config():
    ...
    parser.add_argument(
        ...
        required=False,
        type=int,
        help="The amount of hyperparameter combination random search samples for ray tune.",
        default=200,  # 200 debug
    )
    parser.add_argument("-s", "--storage_path", required=False, type=str, help="Path to store results.", default=None)
    parser.add_argument(
        "-r",
        "--resume",
        required=False,
    ...
    parser.add_argument(
        ...
        default="none",
        help="Method to handle missing values in the input data for transformer models.",
    )
    parser.add_argument(
        "-mmc",
    ...
    parser.add_argument(
        ...
        choices=["pad", "drop"],
        default="pad",
        help="How to handle short subsequences that are shorter than window_len in transformer models.",
    )
    # For all models:
    parser.add_argument(
        "-ucx",
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        choices=["none", "limo", "future_embedding", "recursive"],
        default="future_embedding",
        help="Extrapolation mode for multiple-step forecasting.",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        choices=["none", "sinusoidal", "rope", "learnable"],
        default=None,
        help="Type of positional encoding to use in the transformer model.",
    )
    parser.add_argument(
        "-ml",
        "--max_len",
...
if __name__ == "__main__":
    ...
    run_experiment(args.method, args.graph, args.num_samples, args.storage_path, benedict(vars(args)), args.verbose)
