...
def train_graphlet(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    df_neighs = [
        read_csv_prediction_train(graph_name, "lstm", neigh, predict_dump_dir=predict_dump_dir) for neigh in neighs
    ]
...
def train_transformer_graphlet(config, settings: benedict = benedict({}), verbose: int = 2):
    ...
    predict_dump_dir = DUMP_DIR / "predictions" / settings.get("path_extra_keys", "")
    df_neighs = [
        read_csv_prediction_train(graph_name, "transformer", neigh, predict_dump_dir=predict_dump_dir)
        for neigh in neighs
    ]
    df = merge_graphlet_dfs(df, df_neighs)
...
if __name__ == "__main__":
    ...
    method = "graphlet"  # 'lstm', 'graphlet', 'transformer', or 'transformer_graphlet'
